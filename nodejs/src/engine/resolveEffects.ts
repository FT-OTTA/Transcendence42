...
export function dealsDmg(player1: Hero, player2: Hero, x: number): { actualDmg: number; newArmor: number } {
    if (x <= player2.armor) {
        player2.armor -= x;
        return { actualDmg: 0, newArmor: player2.armor };
    } else {
        const actualDmg = x - player2.armor;
        player1.dmgDealt += actualDmg;
        player2.armor = 0;
        return { actualDmg, newArmor: 0 };
    }
}
...
export function resolveCombat(game: Game, emit?: (event: string, data: any) => void) {
    ...
    for (let i = 1; i <= 8; i++) {
        ...
        // Exclude dead cards; sick creatures stay present (they block but don't attack)
        const raw0 = game.players[0].battlefield[zone];
        const raw1 = game.players[1].battlefield[zone];
        const card0 = raw0 && raw0.currEndurance > 0 ? raw0 : undefined;
        const card1 = raw1 && raw1.currEndurance > 0 ? raw1 : undefined;

        if (!card0 && !card1) continue;

        // A creature that just entered play (sick) blocks but cannot deal damage
        const attacks0 = card0?.type === "creature" && card0.state !== "sick";
        const attacks1 = card1?.type === "creature" && card1.state !== "sick";

        if (!card0) {
            // Player 0's lane is empty — card1 hits hero if it can attack
            if (attacks1) {
                const r = dealsDmg(card1!.owner, game.players[0], card1!.currForce);
                emit?.('combat_event', { type: 'hit_hero', attackerId: card1!.idInGame, targetPlayer: 0, value: card1!.currForce, actualDmg: r.actualDmg, newArmor: r.newArmor, zone });
                resolveTiming(card1!, "on_deal_damage", game.players[0], game, emit);
            }
        } else if (!card1) {
            // Player 1's lane is empty — card0 hits hero if it can attack
            if (attacks0) {
                const r = dealsDmg(card0.owner, game.players[1], card0.currForce);
                emit?.('combat_event', { type: 'hit_hero', attackerId: card0.idInGame, targetPlayer: 1, value: card0.currForce, actualDmg: r.actualDmg, newArmor: r.newArmor, zone });
                resolveTiming(card0, "on_deal_damage", game.players[1], game, emit);
            }
        } else if (card0.type === "creature" && card1.type === "creature") {
            if (attacks0 || attacks1)
                emit?.('combat_event', { type: 'zone_fight', card0Id: card0.idInGame, card1Id: card1.idInGame, zone });

            // card1 attacks card0 (sick card0 still absorbs the hit)
            if (attacks1) {
                if (card0.currEndurance - card1.currForce > 0) {
                    ...
                    const r = dealsDmg(card1.owner, game.players[0], card1.currForce - card0.currEndurance);
                    emit?.('combat_event', { type: 'card_dies', cardId: card0.idInGame, attackerId: card1.idInGame, zone });
                    emit?.('combat_event', { type: 'hit_hero', attackerId: card1.idInGame, targetPlayer: 0, value: card1.currForce - card0.currEndurance, actualDmg: r.actualDmg, newArmor: r.newArmor, zone });
                    card0.currEndurance = 0;
                    resolveTiming(card1, "on_deal_damage", game.players[0], game, emit);
                }
            }

            // card0 attacks card1 (sick card1 still absorbs the hit)
            if (attacks0) {
                if (card1.currEndurance - card0.currForce > 0) {
                    ...
                    const r = dealsDmg(card0.owner, game.players[1], card0.currForce - card1.currEndurance);
                    emit?.('combat_event', { type: 'card_dies', cardId: card1.idInGame, attackerId: card0.idInGame, zone });
                    emit?.('combat_event', { type: 'hit_hero', attackerId: card0.idInGame, targetPlayer: 1, value: card0.currForce - card1.currEndurance, actualDmg: r.actualDmg, newArmor: r.newArmor, zone });
...
const isReady = (c: Card) => !(c.type === "creature" && c.state === "sick") && c.currEndurance > 0;
...
export function resolveEffect(
    ...
    if (eff.target === "all_enemies" || eff.target === "all_allies" || eff.target === "all_board") {
        ...
        if (eff.target === "all_enemies" && opponent)
            targets = Object.values(opponent.battlefield).filter(c => c && isReady(c)) as Card[];
        else if (eff.target === "all_allies")
            targets = Object.values(player.battlefield).filter(c => c && isReady(c)) as Card[];
        else if (eff.target === "all_board")
            for (const p of game.players)
                targets.push(...Object.values(p.battlefield).filter(c => c && isReady(c)) as Card[]);
    ...
    switch (eff.effect) {
        case "ad_mod":
            ...
            emit?.('effect_event', { type: 'ad_mod', targetId: target.idInGame, value, sourceId: payload.cardId });
            break;
        case "def_mod":
            if (target.kind === "hero") return false;
            target.currEndurance += value;
            emit?.('effect_event', { type: 'def_mod', targetId: target.idInGame, value, sourceId: payload.cardId });
            break;
        case "addef_mod":
            if (target.kind === "hero") return false;
            target.currForce += value;
            target.currEndurance += value;
            emit?.('effect_event', { type: 'addef_mod', targetId: target.idInGame, value, sourceId: payload.cardId });
            break;
        case "draw":
            if (target.kind === "card") return false;
            playerDraw(target, value);
            emit?.('effect_event', { type: 'draw', targetId: target.idInGame, value, sourceId: payload.cardId });
            break;
        case "dmg":
            if (target.kind === "hero") {
                const r = dealsDmg(player, target, value);
                emit?.('effect_event', { type: 'dmg_hero', targetId: target.idInGame, value, actualDmg: r.actualDmg, newArmor: r.newArmor, sourceId: payload.cardId });
            } else {
                target.currEndurance -= value;
        ...
        case "armor":
            if (target.kind === "card") return false;
            target.armor += value;
            emit?.('effect_event', { type: 'armor', targetId: target.idInGame, value, newArmor: target.armor, sourceId: payload.cardId });
            break;
        case "runes":
            if (target.kind === "card") return false;
            target.curRunes += value;
            emit?.('effect_event', { type: 'runes', targetId: target.idInGame, value, sourceId: payload.cardId });
            break;
        case "swap":
            ...
            emit?.('effect_event', { type: 'swap', targetId: target.idInGame, target2Id: target2.idInGame, sourceId: payload.cardId });
            break;
        }
        case "destroy":
            if (target.kind === "hero") return false;
        ...
        case "freeze":
            if (target.kind === "hero") return false;
            target.state = "sick";
            emit?.('effect_event', { type: 'freeze', targetId: target.idInGame, sourceId: payload.cardId });
            break;
        case "win":
